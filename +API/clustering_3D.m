function clusters_3D = clustering_3D(He, He_clusters)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[garbage,clusters_3D]=API.kmeans3D(heSlices,k);
...
for slice = 1:numberOfslices
    ...
    for indx =1:k
        ...
        temp2 = API.miprmdebrisn(temp,removeSize);
    ...
    primaryMask(:,:,slice) = API.miprmdebrisn(mask,removeSize); %removing the areas smaller than removeSize from ventilated area (assume to be noise)
...
[garbage,clustersNew]=API.kmeansModified(J,k2);
...
for slice = 1:numberOfslices
    ...
    if (slice==1) | (slice==numberOfslices) | (slice==numberOfslices-1)
        mask = API.miprmdebrisn(mask,removeSize*5); %removing the areas smaller than removeSize from ventilated area (assume to be noise)
    else
        mask = API.miprmdebrisn(mask,removeSize); %removing the areas smaller than removeSize from ventilated area (assume to be noise)
    end
    negativeMask1 = ~mask;
    negativeMask2 = API.miprmdebrisn(negativeMask1,removeSize); %removing the areas smaller than removeSize from non-ventilated (assume to be noise)
    clusterI = clusters_3D(:,:,slice);
