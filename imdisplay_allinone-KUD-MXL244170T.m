...
ProtonMax = prctile(vent_anat(:),99);

%% Display slice of ventilation data
my_slice_vent = squeeze(vent(:,:,slice_vent));
my_slice_anat = squeeze(vent_anat(:,:,slice_vent));
[~,vent_mask2] = Tools.erode_dilate(vent,2,5);
SE = 1;
[x,y,z]=meshgrid(-SE:SE,-SE:SE, -SE:SE);
nhood=x.^2+y.^2+z.^2 <=SE^2;                % structuring element size
se1=strel('arbitrary',nhood);
%erode by one more voxel
%vent_mask=imerode(vent_mask,se1);
my_slice_mask = squeeze(vent_mask(:,:,slice_vent));%ones(size(my_slice_vent));%
my_slice_mask2 = squeeze(vent_mask2(:,:,slice_vent));

my_slice_mask = my_slice_mask + my_slice_mask2;
my_slice_mask(my_slice_mask>0) = 1;
...
[~,~] = Tools.imoverlay(my_slice_anat,my_slice_vent.*my_slice_mask,[00.01,0.8],[0,1.1*ProtonMax],gray,0.9,gca);
colormap(gca,CMap);
InSet = get(gca, 'TightInset');
set(gca, 'Position', [InSet(1:2), 1-InSet(1)-InSet(3), 1-InSet(2)-InSet(4)+.01])
set(gcf,'Position',[1274 449 401 420]);

%.*my_slice_mask
figure('Name','Representative Slice of Vent Image_NoAnat');
[~,~] = Tools.imoverlay(my_slice_anat,my_slice_vent,[0,1],[0,1.1*ProtonMax],gray,1,gca);
colormap(gca,gray);
InSet = get(gca, 'TightInset');
set(gca, 'Position', [InSet(1:2), 1-InSet(1)-InSet(3), 1-InSet(2)-InSet(4)+.01])
set(gcf,'Position',[1274 449 401 420]);

figure('Name','Representative Slice of Anatomic_Only');
[~,~] = Tools.imoverlay(my_slice_anat,my_slice_vent.*my_slice_mask,[0,1],[0,1.1*ProtonMax],gray,0,gca);
colormap(gca,gray);
InSet = get(gca, 'TightInset');
set(gca, 'Position', [InSet(1:2), 1-InSet(1)-InSet(3), 1-InSet(2)-InSet(4)+.01])
set(gcf,'Position',[1274 449 401 420]);
