...
dockercommand = ['docker run ' mapping ' noelmni/antspynet python /mnt/mycode/Analysis_Pipeline/Python_Code/segment_lungs.py /mnt/mydata/' ImName];

status = system(dockercommand);

if ispc
    Mask_name = strrep(ImName,'.nii.gz','_mask.nii.gz');
    copyfile(fullfile(local_path,Mask_name),Orig_ImPath);
end
